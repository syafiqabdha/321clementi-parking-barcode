...
2. **Claim History & Unclaim State Machine:** Redemptions are now primarily keyed to receipt numbers (post-PAN-104), mitigating reliance on vehicle plates. Users who lose their browser session or experience an expired exit countdown can query their claim history via receipt number (or optionally vehicle plate) and initiate an **Unclaim** action. Unclaiming transitions the voucher back to `AVAILABLE` in `voucher_pool` and sets the redemption status to `UNCLAIMED`, immediately releasing the daily 1-redemption constraint for that vehicle.
