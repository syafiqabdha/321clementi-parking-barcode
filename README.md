...
| `PUBLIC_TURNSTILE_SITE_KEY` | ✅ | Frontend (public) | Turnstile site key rendered into the widget. **Read at build time** — the landing page is prerendered, so it must be set in the Vercel build environment. No site key means no widget, no `cf-turnstile-response` token, and therefore a rejected redemption (see the secret key below). | `0x4AAAAAAA...` |
| `CLOUDFLARE_TURNSTILE_SECRET_KEY` | ✅ | Backend | Server-side Turnstile secret for bot protection validation. Set it together with `PUBLIC_TURNSTILE_SITE_KEY` — in production `verifyTurnstileToken()` fails closed when either the secret or the submitted token is missing, so enabling one without the other rejects every redemption with `400 BOT_CHALLENGE_FAILED`. | `0x4AAAAAAA...` |
| `ALLOWED_SITE_DOMAINS` | Optional | Build config | Comma-separated extra hostnames Astro should trust as this deployment's own origin, merged with the defaults in `astro.config.mjs` (`*.vercel.app`, `*.pancatz.com`, `localhost`, `127.0.0.1`). Astro's `checkOrigin` CSRF middleware rejects every same-origin POST whose `Origin` is not allowlisted, so a custom domain must be added here or the redemption POST 403s. Build-time only. | `321clementi.example.com` |
